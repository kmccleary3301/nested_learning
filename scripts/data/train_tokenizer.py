...
def main(
    dataset: str = typer.Option(
        ...
        help="HF dataset name (ignored if manifest set).",
    ),
    split: str = typer.Option(
        "train",
        help="Dataset split (ignored if manifest set).",
    ),
    text_column: str = typer.Option(
        "text",
        help="Text column (ignored if manifest set).",
    ),
    sample_limit: int = typer.Option(
        100_000,
        help="Sample limit per dataset (ignored if manifest set).",
    ),
    ...
    output_dir: Path = typer.Option(
        ...
        help="Directory for tokenizer artifacts.",
    ),
    manifest: Optional[Path] = typer.Option(
        None,
        help="YAML manifest describing multiple datasets.",
    ),
    log_file: Optional[Path] = typer.Option(
        Path("data/mixtures/tokenizer_samples.json"),
        help="Where to log dataset sample stats.",
    ),
) -> None:
    ...
    with tempfile.NamedTemporaryFile("w+", encoding="utf-8", delete=False) as tmp:
        ...
        with tmp_path.open("w", encoding="utf-8") as handle:
            for spec in specs:
                typer.echo(
                    f"[Tokenizer] Streaming {spec.name} ({spec.dataset}) "
                    f"limit={spec.sample_limit}"
                )
